...
export const db = process.env.DATABASE_URL
  ? new Pool({ connectionString: process.env.DATABASE_URL, ssl: { rejectUnauthorized: false }, max: 20 })
  : null;
...
export async function savePlan(userId: string, tierId: string, plan: unknown) {
  if (!db) return null;
  const result = await db.query(
    `INSERT INTO plans (user_id,tier_id,plan_json) VALUES ($1,$2,$3)
     RETURNING id,user_id,tier_id,created_at,updated_at`,
    [userId, tierId, JSON.stringify(plan)]
  );
  return result.rows[0];
}
...
export async function saveProgress(userId: string, workoutKey: string, payload: unknown, completedAt?: string) {
  if (!db) return null;
  const result = await db.query(
    `INSERT INTO workout_progress (user_id,workout_key,payload,completed_at)
     VALUES ($1,$2,$3,$4)
     ON CONFLICT (user_id,workout_key) DO UPDATE SET payload=EXCLUDED.payload, completed_at=EXCLUDED.completed_at
     RETURNING *`,
    [userId, workoutKey, JSON.stringify(payload || {}), completedAt || null]
  );
  return result.rows[0];
}
...
export async function saveReminder(userId: string, reminder: { workoutId: string; scheduledAt: string; status?: string }) {
  if (!db) return null;
  const result = await db.query(
    `INSERT INTO reminders (user_id,workout_id,scheduled_at,status) VALUES ($1,$2,$3,$4) RETURNING *`,
    [userId, reminder.workoutId, reminder.scheduledAt, reminder.status || "pending"]
  );
  return result.rows[0];
}
