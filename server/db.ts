    ...
    const result = await client.query(
      `INSERT INTO rate_limit_buckets(key,window_started_at,request_count,updated_at)
       ...
       ON CONFLICT (key) DO UPDATE SET
         window_started_at=CASE WHEN rate_limit_buckets.window_started_at <= now() - ($2 * interval '1 second') THEN now() ELSE rate_limit_buckets.window_started_at END,
         request_count=CASE WHEN rate_limit_buckets.window_started_at <= now() - ($2 * interval '1 second') THEN 1 ELSE rate_limit_buckets.request_count + 1 END,
         updated_at=now()
...
export async function savePlan(userId: string, tierId: string, plan: unknown) {
  return withContext(userId, async (client) => {
    const result = await client.query(`INSERT INTO plans (user_id,tier_id,plan_json) VALUES ($1,$2,$3) RETURNING id,user_id,tier_id,created_at,updated_at`, [userId, tierId, JSON.stringify(plan)]);
    return result.rows[0];
  });
}
...
export async function listPlans(userId: string) {
  return withContext(userId, async (client) => {
    const result = await client.query(`SELECT id,user_id,tier_id,plan_json,created_at,updated_at FROM plans WHERE user_id=$1 ORDER BY created_at DESC LIMIT 20`, [userId]);
    return result.rows;
  });
}
...
export async function saveProgress(userId: string, workoutKey: string, payload: unknown, completedAt?: string) {
  ...
  return withContext(userId, async (client) => {
    const result = await client.query(`INSERT INTO workout_progress (user_id,workout_key,payload,completed_at) VALUES ($1,$2,$3,$4) ON CONFLICT (user_id,workout_key) DO UPDATE SET payload=EXCLUDED.payload, completed_at=EXCLUDED.completed_at RETURNING *`, [userId, workoutKey, JSON.stringify(payload || {}), completedAt || null]);
    return result.rows[0];
  });
}
...
export async function listProgress(userId: string) {
  return withContext(userId, async (client) => {
    const result = await client.query(`SELECT * FROM workout_progress WHERE user_id=$1 ORDER BY created_at DESC`, [userId]);
    return result.rows;
  });
}
...
export async function saveReminder(userId: string, reminder: { workoutId: string; scheduledAt: string; status?: string }) {
  return withContext(userId, async (client) => {
    const result = await client.query(`INSERT INTO reminders (user_id,workout_id,scheduled_at,status) VALUES ($1,$2,$3,$4) RETURNING *`, [userId, reminder.workoutId, reminder.scheduledAt, reminder.status || "pending"]);
    return result.rows[0];
  });
}
...
export async function listReminders(userId: string) {
  return withContext(userId, async (client) => {
    const result = await client.query(`SELECT * FROM reminders WHERE user_id=$1 ORDER BY scheduled_at ASC`, [userId]);
    return result.rows;
  });
}
...
export async function listNotifications(userId: string) {
  return withContext(userId, async (client) => {
    const result = await client.query(`SELECT * FROM notifications WHERE user_id=$1 ORDER BY created_at DESC LIMIT 100`, [userId]);
    return result.rows;
  });
}

export async function runAsService<T>(fn: (client: PoolClient) => Promise<T>) {
  return withContext(null, fn);
}
