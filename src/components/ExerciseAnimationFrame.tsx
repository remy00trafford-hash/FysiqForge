...
export const ExerciseAnimationFrame:React.FC<ExerciseAnimationFrameProps>=({exerciseId,exerciseName="Exercice",muscleGroup="Mouvement",reps,videoSrc})=>{
 ...
 useEffect(()=>{
   let alive=true;
   setGif(null); setGifFailed(false); setResolved(false);
   findExerciseGif(exerciseId,exerciseName).then(a=>{if(!alive)return;setGif(a?.gifUrl||null);setResolved(true)}).catch(()=>{if(alive)setResolved(true)});
   return()=>{alive=false};
 ...
 if(!resolved)return <div className="relative flex h-full min-h-[210px] w-full items-center justify-center overflow-hidden rounded-2xl bg-[#070B10]"><div className="rounded-xl border border-white/10 bg-black/60 px-4 py-3 text-[10px] font-black uppercase tracking-wider text-white/70">Chargement de l’animation…</div></div>;
 return <div className="relative h-full min-h-[210px] w-full overflow-hidden rounded-2xl bg-[#070B10]"><PremiumWhiteHumanExerciseAnimation exerciseId={exerciseId} exerciseName={exerciseName} muscleGroup={muscleGroup}/><div className="pointer-events-none absolute inset-x-2 top-2 flex items-center justify-between gap-2 sm:inset-x-3"><div className="rounded-xl border border-white/10 bg-black/65 px-2.5 py-2 text-[9px] font-black uppercase tracking-wider text-white/80 backdrop-blur-md">Animation humaniste blanche • FysiqForge</div>{reps&&<div className="rounded-xl border border-white/10 bg-black/65 px-2.5 py-2 text-[10px] font-black text-white">{reps}</div>}</div></div>;
};