...
export const NutritionGuideModal: React.FC<NutritionGuideModalProps> = ({ userAnswers, onClose }) => {
  ...
  const meals = mealTemplates[mode];
    ...
    <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-md flex items-center justify-center p-4 overflow-y-auto">
      <div className="bg-[#16161E] border border-white/20 rounded-3xl max-w-3xl w-full p-6 sm:p-8 space-y-6 text-white shadow-2xl relative my-8">
        ...
        <div className="flex items-center justify-between bg-[#121218] border border-white/10 rounded-2xl p-3">
          ...
          <div className="text-center"><p className="text-xs text-gray-500 uppercase font-bold">Suivi</p><p className="font-black">{dayLabel}</p></div>
        ...
        <div className="bg-[#121218] border border-white/10 rounded-2xl p-4">
          ...
          <div className="space-y-2">
            {meals.map((meal, idx) => {
              ...
              return <button key={meal.title} onClick={() => toggleMeal(idx)} className={`w-full text-left p-3 rounded-xl border transition-colors ${checked ? "bg-emerald-500/10 border-emerald-500/30" : "bg-white/[0.02] border-white/5 hover:border-white/15"}`}>
        ...
        <div className="bg-[#121218] border border-white/10 rounded-2xl p-4">
          ...
          <div className="flex gap-2 flex-wrap">{Array.from({ length: waterSteps }).map((_, idx) => <button key={idx} onClick={() => setWater((w) => idx < Math.round(w * 4) ? Math.max(0, idx / 4) : (idx + 1) / 4)} className={`h-8 w-8 rounded-lg border text-[10px] font-bold ${idx < Math.round(water * 4) ? "bg-blue-500/30 border-blue-400/40 text-blue-200" : "bg-white/5 border-white/10 text-gray-500"}`}>{idx + 1}</button>)}</div>
          <p className="text-[10px] text-gray-500 mt-2">Objectif indicatif : répartis l'hydratation sur la journée.</p>
        </div>
