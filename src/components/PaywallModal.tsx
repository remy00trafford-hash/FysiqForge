...
export const PaywallModal: React.FC<PaywallModalProps> = ({
  ...
  const handleOpenCheckout = (tier: PlanTierConfig, currency: "FCFA" | "USD" | "EUR", amount: number) => {
    if (!userEmail.trim() || !userName.trim()) {
      return; // Le bouton "Payer" reste désactivé plus bas tant que le compte n'est pas rempli
    }
        ...
        {isGoogleSignedIn ? (
          <div className="flex items-center justify-between p-3.5 bg-emerald-950/30 border border-emerald-500/40 rounded-xl text-xs text-emerald-300 font-semibold">
            <div className="flex items-center gap-2">
              ...
              <span>Compte prêt : <strong>{userName || userEmail}</strong></span>
            </div>
            <button
          ...
          <div className="space-y-3">
            ...
            <button
              ...
              className="w-full bg-[#FF5500] hover:bg-[#FF6A1F] disabled:opacity-40 disabled:cursor-not-allowed text-white font-bold px-4 py-3 rounded-xl text-sm transition-colors cursor-pointer"
            >
              Valider mon compte
            </button>
          </div>
