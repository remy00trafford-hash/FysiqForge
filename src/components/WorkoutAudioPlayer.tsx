...
/**
 * Music is a paid-plan entitlement. An empty playlist means the feature is not
 * included and the player must not silently fall back to a public/default track.
 */
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
        <div className="flex items-center gap-3 min-w-0">
          <div className="w-12 h-12 rounded-xl overflow-hidden bg-black border border-white/10 shrink-0 flex items-center justify-center">
            {playlist?.coverUrl ? (
              ...
              <Music2 className="w-5 h-5 text-[#FF5500]" />
            )}
          </div>
          <div className="min-w-0">
            <p className="text-[10px] font-black text-[#FF5500] uppercase tracking-wider">Musique de séance</p>
            <p className="font-extrabold text-sm truncate">{track.title || playlist.title}</p>
        ...
        <div className="flex items-center gap-2 shrink-0">
          <a
            ...
            className="bg-[#FF5500] hover:bg-[#FF6611] text-white font-black text-xs px-4 py-2.5 rounded-xl flex items-center gap-2"
          >
